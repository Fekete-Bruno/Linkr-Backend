...
async function SignUp(req, res) {
  try {
    console.log("aaa");
    const searchByEmail = (
      await signRepository.SelectUserByEmail(req.body.email)
    ).rows;
...
async function SignIn(req, res) {
  try {
    ...
    const compare = bcrypt.compareSync(
      res.locals.body.password,
      searchByEmail[0].password
    );
    if (compare) {
      const token = uuidv4();
      ...
      res.status(200).send({
        token: token,
        img: searchByEmail[0].img,
        id: searchByEmail[0].id,
...
async function SignOutAll(req, res) {
  try {
    await signRepository.DeleteAllSessionsByUserId(
      res.locals.searchToken[0].userId
    );
    res.sendStatus(200);
  } catch (error) {
    res.status(500).send(error);
